...
class ClientProfileControllerImpl[F[_] : Async : Concurrent : Logger](
  ...
  val routes: HttpRoutes[F] = HttpRoutes.of[F] {
    ...
    case req @ GET -> Root / "client" / "profile" / "health" =>
      Logger[F].debug(s"[ClientProfileControllerImpl] GET - Health check for backend ClientProfileControllerImpl service") *>
        Ok(GetResponse("/dev-user-service/health", "I am alive").asJson)

    case req @ GET -> Root / "client" / "profile" / "data" / userId =>
      (
        Logger[F].debug(s"[ClientProfileControllerImpl][/user/data/$userId] GET - Attempting to retrieve user details") *>
          Async[F].pure(extractCookieSessionToken(req))
      ).flatMap {
        case Some(cookieToken) =>
          withValidSession(userId, cookieToken) {
            Logger[F].debug(s"[ClientProfileControllerImpl] GET - Authenticated for userId $userId") *>
              userService.getUser(userId).flatMap {
                case Some(user) =>
                  Logger[F].debug(s"[ClientProfileControllerImpl] GET - Found user ${userId.toString()}") *>
                    Ok(user.asJson)
                case None =>
        ...
        case None =>
          Logger[F].debug(s"[ClientProfileControllerImpl] GET - Unauthorised") *>
            Unauthorized(`WWW-Authenticate`(Challenge("Bearer", "api")), "Missing Cookie")
      }

    case req @ PUT -> Root / "client" / "profile" / "data" / "update" / userId =>
      extractCookieSessionToken(req) match {
        case Some(headerToken) =>
          withValidSession(userId, headerToken) {
            Logger[F].debug(s"[ClientProfileControllerImpl] PUT - Updating user with ID: $userId") *>
              req.decode[UpdateUserData] { request =>
                userService.updateUserData(userId, request).flatMap {
                  case Valid(response) =>
                    Logger[F].debug(s"[ClientProfileControllerImpl] PUT - Successfully updated user for ID: $userId") *>
                      Ok(UpdatedResponse(UpdateSuccess.toString, s"User $userId updated successfully").asJson)
                  case Invalid(errors) =>
                    Logger[F].debug(s"[ClientProfileControllerImpl] PUT - Validation failed for user update: ${errors.toList}") *>
    ...
    case req @ DELETE -> Root / "client" / "profile" / "data" / "delete" / userId =>
      extractCookieSessionToken(req) match {
        case Some(headerToken) =>
          withValidSession(userId, headerToken) {
            Logger[F].debug(s"[ClientProfileControllerImpl] DELETE - Attempting to delete user") *>
              userService.deleteUser(userId).flatMap {
                case Valid(response) =>
                  Logger[F].debug(s"[ClientProfileControllerImpl] DELETE - Successfully deleted user for $userId") *>
...
object ClientProfileController {
  def apply[F[_] : Async : Concurrent](
    ...
    new ClientProfileControllerImpl[F](userService, sessionCache)
}
